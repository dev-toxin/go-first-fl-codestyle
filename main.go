...
func attack(charName, charClass string) string {
	if charClass == "warrior" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, 5+randomise(3, 5))
	}

	if charClass == "mage" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, 5+randomise(5, 10))
	}

	if charClass == "healer" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, 5+randomise(-3, -1))
	}
...
func defence(charName, charClass string) string {
	if charClass == "warrior" {
		return fmt.Sprintf("%s блокировал %d урона.", charName, 10+randomise(5, 10))
	}
	if charClass == "mage" {
		return fmt.Sprintf("%s блокировал %d урона.", charName, 10+randomise(-2, 2))
	}
	if charClass == "healer" {
		return fmt.Sprintf("%s блокировал %d урона.", charName, 10+randomise(2, 5))
	}
	return "неизвестный класс персонажа"
}
